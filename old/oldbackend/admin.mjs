...
app.post("/api/admin/logout", async (req, res) => {
    const cookLI = getCookie(req, `${config.cookiePrefix}LI`);
    if (cookLI && cookLI.logout_token)
        await auth.logout({ logout_token: cookLI.logout_token });
    clearCookie(res, `${config.cookiePrefix}LI`);
    clearCookie(res, `${config.cookiePrefix}II`);
    return res.send();
})

app.post("/api/admin/data", async (req, res) => {
    try {
        const session = getCookie(req, `${config.cookiePrefix}LI`);
        if (!await userAuthenticate(session, res)) return;
        if (!await userAuthorize(session, res)) return;

        const rows = (await knex.raw(`SELECT * FROM apps ORDER BY id`)).rows;
...
app.post("/api/admin/submit", async (req, res) => {
    try {
        const cookie = getCookie(req, `${config.cookiePrefix}LI` );
        ...
        if (req.body.add == "new") {
            await knex.raw(`INSERT INTO apps (name, login_callback, skipprofile, groups) VALUES ( :name, :login_callback, :login_profile, :groups )`, {
                name:           req.body.name,
        ...
        if (req.body.save) {
            ...
            await knex.raw(`UPDATE apps SET ${Object.keys(opts).map(x => `${x} = :${x}`).join(', ')} WHERE id = :id `, {
                name:           req.body.name,
                login_callback: req.body.login_callback,
        ...
        if (req.body.del)
            await knex.raw(`DELETE FROM apps WHERE id = :id`, { id: req.body.del });
