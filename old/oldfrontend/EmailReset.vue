<widget>
{
    "name": "Roon Email Reset1",
    "description": "The email reset component for https://roonlabs.com/reset-email",
    "tags": [ 'custom', 'web' ],
    "strings": {
        "label_email":            { "value": "Email" },
        "submit_button":          { "value": "Reset" },
        "success_check_email":    { "value": "An email reset link has been sent. Please check the email you entered for a link to reset it." },
...
export default {
    ...
    mounted() {
        ...
        if (this.qs.code) {
            window.location.href = "/reset-email2?code=" +  encodeURIComponent(this.qs.code);
            return;
        }
